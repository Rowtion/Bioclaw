...
|------|------|---------|----------|-------------|
| X_train | array | None | Yes | Training time series data (n_samples, n_channels, n_timepoints) |
| y_train | array | None | Yes | Training labels for supervised tasks |
| X_test | array | None | No | Test time series data for evaluation |
| y_test | array | None | No | Test labels for evaluation |
| n_kernels | integer | 10000 | No | Number of kernels for ROCKET-based classifiers |
| n_clusters | integer | 3 | No | Number of clusters for clustering algorithms |
| distance | string | "dtw" | No | Distance metric (dtw, euclidean, manhattan, etc.) |
| order | tuple | (1, 1, 1) | No | ARIMA model order (p, d, q) |
| window_size | integer | 50 | No | Window size for anomaly detection and segmentation |
| n_epochs | integer | 100 | No | Number of training epochs for deep learning |
| batch_size | integer | 32 | No | Batch size for training |
| task | string | "classification" | No | ML task type (classification, regression, clustering, forecasting, anomaly_detection, segmentation, similarity_search) |
| algorithm | string | "RocketClassifier" | No | Algorithm to use for the specified task |
