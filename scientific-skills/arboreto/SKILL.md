...
|------|------|---------|----------|-------------|
| expression_data | DataFrame | None | Yes | Gene expression matrix (observations × genes) |
| tf_names | list | None | No | List of transcription factor names to restrict inference |
| seed | integer | None | No | Random seed for reproducibility |
| client_or_address | string | None | No | Dask client or scheduler address for distributed computing |
| algorithm | string | "grnboost2" | No | Algorithm choice (grnboost2, genie3) |
| importance_threshold | float | 0.5 | No | Threshold for filtering regulatory links |
